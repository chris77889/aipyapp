...
class WebToolsPlugin(TaskPlugin):
    ...
    def init(self):
        ...
        self.headers = {'User-Agent': self.user_agent, **self.config.get('default_headers', {})}
    ...
    def fn_fetch_webpage(self, url: str, extract_text: bool = True) -> Dict[str, Any]:
        ...
        Examples:
            >>> fetch_webpage("https://www.baidu.com")
            ...
            >>> fetch_webpage("https://www.baidu.com", extract_text=False)
    ...
    def fn_http_request(self, url: str, method: str = "GET", headers: Optional[Dict[str, str]] = None, params: Optional[Dict[str, Any]] = None, json_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Examples:
            ...
            >>> http_request("https://www.baidu.com", method="POST", json_data={"name": "John", "age": 30})
            {'success': True, 'status_code': 200, 'headers': {'Content-Type': 'text/html; charset=utf-8'}, 'content': 'Baidu', 'elapsed': '0.000000s'}
            >>> http_request("https://www.baidu.com")
    ...
    def fn_analyze_url(self, url: str) -> Dict[str, str]:
        ...
        Examples:
            >>> analyze_url("https://user:pass@example.com:8080/path/to/page?query=value#section")
            {'scheme': 'https', 'netloc': 'user:pass@example.com:8080', 'hostname': 'example.com', 'port': '8080', 'path': '/path/to/page', 'params': '', 'query': 'query=value', 'fragment': 'section', 'username': 'user', 'password': '***'}
    ...
    def fn_check_url_status(self, url: str) -> Dict[str, Any]:
        ...
        Examples:
            >>> check_url_status("https://www.baidu.com")
            {'accessible': True, 'status_code': 200, 'content_type': 'text/html', 'content_length': '12345', 'last_modified': 'Mon, 13 Feb 2026 00:00:00 GMT', 'server': 'baidu/1.0'}
            >>> check_url_status("https://this-domain-does-not-exist-12345.com")
            {'accessible': False, 'error': 'Failed to establish connection'}
        """
    ...
    def _fetch_webpage(self, url: str, extract_text: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_length and int(content_length) > self.max_content_length:
                return {"success": False, "error": f"Content too large ({content_length} bytes), exceeds limit ({self.max_content_length} bytes)"}

            response.raise_for_status()

            result = {"success": True, "url": url, "status_code": response.status_code, "headers": dict(response.headers), "content_type": response.headers.get('content-type', ''), "encoding": response.encoding}
    ...
    def _http_request(self, url: str, method: str, headers: Optional[Dict], params: Optional[Dict], json_data: Optional[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            kwargs = {'headers': request_headers, 'timeout': self.timeout, 'params': params}
            ...
            return {"success": True, "status_code": response.status_code, "headers": dict(response.headers), "content": response.text, "elapsed": str(response.elapsed)}
    ...
    def _check_url_status(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"accessible": True, "status_code": response.status_code, "content_type": response.headers.get('content-type', ''), "content_length": response.headers.get('content-length', ''), "last_modified": response.headers.get('last-modified', ''), "server": response.headers.get('server', '')}
        except Exception as e:
